...
function najax(uri, options, callback) {
	...
	o.headers.cookie = o.headers.cookie || cookie.cookieStringForRequest(l.hostname,l.pathname, ssl) || ''
    ...
    var req = (ssl ? https : http).request(options, function(res) {
        ...
        function dataHandler(data) {
            ...
            if (isSuccess) {
                ...
                // get cookies
                var setCookie = jqXHR.getResponseHeader('set-cookie');
				if(setCookie)
                setCookie.forEach(function(e) {
                    try {
                        // only use valid cookies with domain fields
                        if (e.indexOf('domain') > -1) {
                            cookie.addFromHeaders(e);
                        }
                    } catch (err) {}
                });
...
function handleMethod(method) {
    najax[method.toLowerCase()] = function methodHandler(uri, options, callback) {
        return najax(defaultsDeep(parseOptions(uri, options, callback), {
            method: method
        }))
    }
}
